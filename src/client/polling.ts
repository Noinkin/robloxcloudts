import { RobloxEvents, ResourceAction as PollingEvents, type RobloxClient, ResourceEvents } from "./client.js";
...
export class ResourcePoller<T = any> {
    ...
    constructor(client: RobloxClient, config: Omit<PollingConfig<T>, 'cache' | 'interval'>) {
        this.client = client
        this.config = {
            ...config,
            cache: new Map(),
    ...
    start(): void {
        if(this.isPolling) {
            this.client.emit(RobloxEvents.Warn, `Poller "${this.config.key}" is already running`)
            return;
        }
        ...
        this.client.emit(RobloxEvents.Debug, `Started Polling "${this.config.key}" every ${interval}ms`)
    }
    ...
    stop(): void {
        if(this.config.interval) {
            ...
            this.client.emit(RobloxEvents.Debug, `Stopped Polling "${this.config.key}"`)
        }
    }
        ...
        try {
            ...
            for(const item of items) {
                ...
                if(!cached) {
                    ...
                    this.config.cache.set(id, item)
                    this.emitEvent(PollingEvents.Update, { old: cached, new: item })
                }
            }
...
export class PollingManager{
    ...
    startPolling<T = any>(resource: ResourceEvents, resourceId: string, fetchFunction: () => Promise<T[]>, options: PollingOptions): ResourcePoller<T> {
        if(!options) options = {}
